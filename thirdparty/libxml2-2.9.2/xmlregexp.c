...
xmlAutomataNewNegTrans(xmlAutomataPtr am, xmlAutomataStatePtr from,
    ...
    _snprintf((char *) err_msg, 199, "not %s", (const char *) atom->valuep);
    err_msg[199] = 0;
...
xmlExpDumpInt(xmlBufferPtr buf, xmlExpNodePtr expr, int glob) {
    ...
    switch (expr->type) {
		...
		rep[1] = 0;
	        ...
	        _snprintf(rep, 39, "{%d}", expr->exp_min);
	    } else if (expr->exp_max < 0) {
	        _snprintf(rep, 39, "{%d,inf}", expr->exp_min);
	    } else {
	        _snprintf(rep, 39, "{%d,%d}", expr->exp_min, expr->exp_max);
	    }
	    rep[39] = 0;
