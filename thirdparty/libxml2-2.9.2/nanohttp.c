...
retry:
    ...
    if (proxy) {
	if (ctxt->port != 80) {
	    p += _snprintf( p, blen - (p - bp), "%s http://%s:%d%s",
			method, ctxt->hostname,
			ctxt->port, ctxt->path );
	}
	else
	    p += _snprintf( p, blen - (p - bp), "%s http://%s%s", method,
			ctxt->hostname, ctxt->path);
    }
    else
	p += _snprintf( p, blen - (p - bp), "%s %s", method, ctxt->path);

    if (ctxt->query != NULL)
	p += _snprintf( p, blen - (p - bp), "?%s", ctxt->query);

    if (ctxt->port == 80) {
        p += _snprintf( p, blen - (p - bp), " HTTP/1.0\r\nHost: %s\r\n",
		    ctxt->hostname);
    } else {
        p += _snprintf( p, blen - (p - bp), " HTTP/1.0\r\nHost: %s:%d\r\n",
...
#ifdef HAVE_ZLIB_H
    p += _snprintf(p, blen - (p - bp), "Accept-Encoding: gzip\r\n");
#endif

    if (contentType != NULL && *contentType)
	p += _snprintf(p, blen - (p - bp), "Content-Type: %s\r\n", *contentType);

    if (headers != NULL)
	p += _snprintf( p, blen - (p - bp), "%s", headers );

    if (input != NULL)
	_snprintf(p, blen - (p - bp), "Content-Length: %d\r\n\r\n", ilen );
    else
	_snprintf(p, blen - (p - bp), "\r\n");
