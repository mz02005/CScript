...
xmlFatalErr(xmlParserCtxtPtr ctxt, xmlParserErrors error, const char *info)
{
    ...
    if (info == NULL)
        _snprintf(errstr, 128, "%s\n", errmsg);
    else
        _snprintf(errstr, 128, "%s: %%s\n", errmsg);
    if (ctxt != NULL)
	ctxt->errNo = error;
...
xmlParseReference(xmlParserCtxtPtr ctxt) {
	...
	if (ctxt->charset != XML_CHAR_ENCODING_UTF8) {
		...
		if ((hex == 'x') || (hex == 'X'))
		    _snprintf((char *)out, sizeof(out), "#x%X", value);
		else
		    _snprintf((char *)out, sizeof(out), "#%d", value);
...
encoding_error:
	...
	_snprintf(buffer, 149, "Bytes: 0x%02X 0x%02X 0x%02X 0x%02X\n",
