...
xmlNanoFTPSendUser(void *ctx) {
    ...
    if (ctxt->user == NULL)
	_snprintf(buf, sizeof(buf), "USER anonymous\r\n");
    else
	_snprintf(buf, sizeof(buf), "USER %s\r\n", ctxt->user);
    buf[sizeof(buf) - 1] = 0;
    len = strlen(buf);
#ifdef DEBUG_FTP
...
xmlNanoFTPSendPasswd(void *ctx) {
    ...
    if (ctxt->passwd == NULL)
	_snprintf(buf, sizeof(buf), "PASS anonymous@\r\n");
    else
	_snprintf(buf, sizeof(buf), "PASS %s\r\n", ctxt->passwd);
    buf[sizeof(buf) - 1] = 0;
    len = strlen(buf);
#ifdef DEBUG_FTP
    xmlGenericError(xmlGenericErrorContext, "%s", buf);
...
xmlNanoFTPConnect(void *ctx) {
	...
	switch (proxyType) {
	    ...
	    case 1:
		/* Using SITE command */
		_snprintf(buf, sizeof(buf), "SITE %s\r\n", ctxt->hostname);
                buf[sizeof(buf) - 1] = 0;
                len = strlen(buf);
#ifdef DEBUG_FTP
		xmlGenericError(xmlGenericErrorContext, "%s", buf);
		...
		if (ctxt->user == NULL)
		    _snprintf(buf, sizeof(buf), "USER anonymous@%s\r\n",
			           ctxt->hostname);
		else
		    _snprintf(buf, sizeof(buf), "USER %s@%s\r\n",
...
xmlNanoFTPCwd(void *ctx, const char *directory) {
     ...
     */
    _snprintf(buf, sizeof(buf), "CWD %s\r\n", directory);
    buf[sizeof(buf) - 1] = 0;
    len = strlen(buf);
#ifdef DEBUG_FTP
    xmlGenericError(xmlGenericErrorContext, "%s", buf);
...
xmlNanoFTPDele(void *ctx, const char *file) {
    ...
    _snprintf(buf, sizeof(buf), "DELE %s\r\n", file);
    buf[sizeof(buf) - 1] = 0;
    len = strlen(buf);
#ifdef DEBUG_FTP
    xmlGenericError(xmlGenericErrorContext, "%s", buf);
...
#ifdef SUPPORT_IP6
	if ((ctxt->ftpAddr).ss_family == AF_INET6)
	    _snprintf (buf, sizeof(buf), "EPSV\r\n");
	else
#endif
	    _snprintf (buf, sizeof(buf), "PASV\r\n");
        len = strlen (buf);
#ifdef DEBUG_FTP
	xmlGenericError(xmlGenericErrorContext, "%s", buf);
#endif
...
xmlNanoFTPList(void *ctx, ftpListCallback callback, void *userData,
	...
	if (ctxt->dataFd == INVALID_SOCKET)
	    return(-1);
	_snprintf(buf, sizeof(buf), "LIST -L\r\n");
    } else {
	if (filename[0] != '/') {
	    if (xmlNanoFTPCwd(ctxt, ctxt->path) < 1)
		return(-1);
	...
	if (ctxt->dataFd == INVALID_SOCKET)
	    return(-1);
	_snprintf(buf, sizeof(buf), "LIST -L %s\r\n", filename);
    }
    buf[sizeof(buf) - 1] = 0;
    len = strlen(buf);
#ifdef DEBUG_FTP
...
xmlNanoFTPGetSocket(void *ctx, const char *filename) {
    ...
    _snprintf(buf, sizeof(buf), "TYPE I\r\n");
    len = strlen(buf);
#ifdef DEBUG_FTP
    xmlGenericError(xmlGenericErrorContext, "%s", buf);
#endif
