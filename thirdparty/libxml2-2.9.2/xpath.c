...
xmlXPathFormatNumber(double number, char buffer[], int buffersize)
{
	...
	if (buffersize > (int)sizeof("Infinity"))
	    _snprintf(buffer, buffersize, "Infinity");
	break;
    case -1:
	if (buffersize > (int)sizeof("-Infinity"))
	    _snprintf(buffer, buffersize, "-Infinity");
	...
	if (xmlXPathIsNaN(number)) {
	    ...
	    */
		...
		size = _snprintf(work, sizeof(work),"%*.*e",
	    ...
	    /* Remove leading spaces sometimes inserted by _snprintf */
	    while (work[0] == ' ') {
