...
xmlSchemaFormatFloat(double number, char buffer[], int buffersize)
{
	...
	if (buffersize > (int)sizeof("INF"))
	    _snprintf(buffer, buffersize, "INF");
	break;
    case -1:
	if (buffersize > (int)sizeof("-INF"))
	    _snprintf(buffer, buffersize, "-INF");
	...
	if (xmlXPathIsNaN(number)) {
		...
		_snprintf(buffer, buffersize, "NaN");
	} else if (number == 0) {
	    _snprintf(buffer, buffersize, "0.0E0");
	} else {
    ...
    *retValue = NULL;
	...
	case XML_SCHEMAS_DECIMAL:
		...
		*retValue = xmlStrdup(BAD_CAST "0.0");
		...
		if (dec.hi != 0)
		    _snprintf(offs, bufsize - (offs - buf),
	...
	case XML_SCHEMAS_UINT:
		...
		*retValue = xmlMalloc(bufsize);
		...
		if (dec.hi != 0) {
		    if (dec.sign)
			_snprintf((char *) *retValue, bufsize,
			    "-%lu%lu%lu", dec.hi, dec.mi, dec.lo);
		    else
			_snprintf((char *) *retValue, bufsize,
			    "%lu%lu%lu", dec.hi, dec.mi, dec.lo);
		    ...
		    if (dec.sign)
			_snprintf((char *) *retValue, bufsize, "-%lu", dec.lo);
		    else
			_snprintf((char *) *retValue, bufsize, "%lu", dec.lo);
		}
	    }
	...
	case XML_SCHEMAS_DURATION: {
		...
		*/
		...
		if ((val->value.dur.mon < 0) || (val->value.dur.sec < 0))
		    _snprintf(buf, 100, "P%luY%luM%luDT%luH%luM%.14gS",
	...
	case XML_SCHEMAS_GYEAR: {
		...
		_snprintf(buf, 30, "%04ld", val->value.date.year);
		*retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
	    }
	    break;
	case XML_SCHEMAS_GMONTH: {
		...
		*retValue = xmlMalloc(8);
		if (*retValue == NULL)
		    return(-1);
		_snprintf((char *) *retValue, 8, "--%02u-%02u",
	...
	case XML_SCHEMAS_FLOAT: {
		...
		* the whole range.
		*/
		_snprintf(buf, 30, "%01.14e", val->value.f);
		*retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
	    }
	    break;
	case XML_SCHEMAS_DOUBLE: {
		...
		* the whole range.
		*/
		_snprintf(buf, 40, "%01.14e", val->value.d);
		*retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
	    }
	    break;
	default:
