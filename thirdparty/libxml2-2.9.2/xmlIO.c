...
xmlGzfileOpenW (const char *filename, int compression) {
    ...
    _snprintf(mode, sizeof(mode), "wb%d", compression);
    if (!strcmp(filename, "-")) {
...
xmlCreateZMemBuff( int compression ) {
    ...
    hdr_lgth = _snprintf( (char *)buff->zbuff, buff->size,
			"%c%c%c%c%c%c%c%c%c%c",
...
xmlIOHTTPCloseWrite( void * context, const char * http_mthd ) {
	...
	if ( http_ctxt != NULL ) {
	    ...
	    */
	    ...
	    if ( dump_name != NULL ) {
		(void)_snprintf( buffer, sizeof(buffer), "%s.content", dump_name );
		...
		(void)_snprintf( buffer, sizeof(buffer), "%s.reply", dump_name );
		tst_file = fopen( buffer, "wb" );
		if ( tst_file != NULL ) {
		    xmlGenericError( xmlGenericErrorContext,
