	...
	*/
	if ((facetType == XML_SCHEMA_FACET_LENGTH) ||
	    ...
	    */
	    ...
	    _snprintf(len, 24, "%lu", xmlSchemaGetFacetValueAsULong(facet));
	    _snprintf(actLen, 24, "%lu", length);
                         ...
                         xmlNodePtr node, int topLevel)
{
	...
	*/
#ifdef ENABLE_NAMED_LOCALS
        _snprintf(buf, 39, "#ST%d", ctxt->counter++ + 1);
                          ...
                          xmlNodePtr node, int topLevel)
{
	...
	*/
#ifdef ENABLE_NAMED_LOCALS
        _snprintf(buf, 39, "#CT%d", ctxt->counter++ + 1);
	type = xmlSchemaAddType(ctxt, schema,
			  ...
			  xmlSchemaTypePtr type)
{
	...
	*/
	if ((WXS_IS_COMPLEX(baseType)) &&
	    ...
	    */
#ifdef ENABLE_NAMED_LOCALS
	    _snprintf(buf, 29, "#scST%d", ++(pctxt->counter));
	...
	if (iattr->state == XML_SCHEMAS_ATTR_DEFAULT) {
		...
		*/
		...
		if (iattr->nsName == NULL) {
			...
			*/
			do {
			    _snprintf((char *) prefix, 12, "p%d", counter++);
