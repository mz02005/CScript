...
xmlGetNodePath(const xmlNode *node)
{
    ...
    do {
		...
		if (cur->ns->prefix != NULL) {
                    _snprintf(nametemp, sizeof(nametemp) - 1, "%s:%s",
	    ...
	    */
	    ...
	    next = cur->parent;
		    ...
		    _snprintf(nametemp, sizeof(nametemp) - 1, "%s",
			(char *)cur->name);
                nametemp[sizeof(nametemp) - 1] = 0;
                name = nametemp;
            }
        ...
        if (occur == 0)
            _snprintf((char *) buf, buf_len, "%s%s%s",
                     sep, name, (char *) buffer);
        else
            _snprintf((char *) buf, buf_len, "%s%s[%d]%s",
                     sep, name, occur, (char *) buffer);
        _snprintf((char *) buffer, buf_len, "%s", (char *)buf);
        cur = next;
    } while (cur != NULL);
    xmlFree(buf);
    return (buffer);
...
xmlNewReconciliedNs(xmlDocPtr doc, xmlNodePtr tree, xmlNsPtr ns) {
     ...
     */
    if (ns->prefix == NULL)
	_snprintf((char *) prefix, sizeof(prefix), "default");
    else
	_snprintf((char *) prefix, sizeof(prefix), "%.20s", (char *)ns->prefix);

    def = xmlSearchNs(doc, tree, prefix);
    while (def != NULL) {
        if (counter > 1000) return(NULL);
	if (ns->prefix == NULL)
	    _snprintf((char *) prefix, sizeof(prefix), "default%d", counter++);
	else
	    _snprintf((char *) prefix, sizeof(prefix), "%.20s%d",
