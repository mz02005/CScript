...
namespace compiler {
	...
	FunctionStatement::FunctionStatement()
		: mIsTopLevel(true)
		, mIsAnonymousFuncInExpression(false)
	{
	}
	...
	int FunctionStatement::Compile(Statement *parent, SimpleCScriptEngContext *context)
	{
		...
		FunctionStatement *parentFun = nullptr;
		if (!mIsAnonymousFuncInExpression) {
			parentFun = GetParentFunction();
			if (!parentFun)
			{
		...
		if (parentFun || mIsAnonymousFuncInExpression)
		{
			if ((parseResult = context->GetNextSymbol(symbol)) != 0)
				RETHELP(parseResult);
			if (symbol.symbolOrig != "(")
	...
	int FunctionStatement::GenerateNamedFunctionCode(CompileResult *compileResult)
	{
		GenerateInstructionHelper gih(compileResult);
	...
	int FunctionStatement::GenerateAnonymousFunctionCode(CompileResult *compileResult)
	{
		int result;
		uint32_t x;
		uint32_t jumpToFuncEnd;
		uint32_t functionStart, functionEnd;
		...
		if ((result = GenerateLocalVariableInstruction(compileResult)) < 0)
			return result;
		if ((result = mFunctionBody.GenerateInstruction(compileResult)) < 0)
			return result;

		functionEnd = compileResult->SaveCurrentCodePosition();
		gih.SetCode(functionStart, (functionEnd - functionStart) * 4);
		gih.SetCode(jumpToFuncEnd, functionEnd);
		gih.SetCode(x, functionStart);
	...
	{
		return mIsAnonymousFuncInExpression ? GenerateAnonymousFunctionCode(compileResult)
			: GenerateNamedFunctionCode(compileResult);
	}
	...
	IMPLEMENT_OBJINFO(FunctionDefinationExpress,ExpressionNode)

	FunctionDefinationExpress::FunctionDefinationExpress(const std::string &funcName)
	: mName(funcName)
	, mFuncStatement(new FunctionStatement(mName))
	{
		mFuncStatement->mIsAnonymousFuncInExpression = true;
	}

	FunctionDefinationExpress::~FunctionDefinationExpress()
	{
		delete mFuncStatement;
	}

	int FunctionDefinationExpress::Compile(SimpleCScriptEngContext *context)
	{
		int result;
		Symbol symbol;

		if ((result = mFuncStatement->Compile(nullptr, context)) < 0)
			return result;

		return result;
	}

	int FunctionDefinationExpress::GenerateInstruction(Statement *parent, CompileResult *compileResult)
	{
